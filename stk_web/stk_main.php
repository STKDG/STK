...
<head>
    ...
    <?php
        function loadSignedIn($user_id){
    ?>
    <script>
        
        var toLoad2 = document.getElementById('toLoad2');
        var toLoad3 = document.getElementById('toLoad3');
        
        var MyAccount = document.createElement('button');
        MyAccount.className = "login";
        MyAccount.style.border = "3px solid orange";
        MyAccount.addEventListener('click', function(){
            location.href='stk_MyAccount.php'
        });
        var MyAccountText = document.createTextNode('<?=$user_id?>의 계정 설정');
        toLoad2.appendChild(MyAccount).appendChild(MyAccountText);
        
        var LogOut = document.createElement('button');
        LogOut.className = "login";
        LogOut.style.border = "3px solid orange";
        var LogOutText = document.createTextNode('로그아웃');
        toLoad3.appendChild(LogOut).appendChild(LogOutText);
    </script>
    <?php
        }   
    ...
    <script>
        ...
        inputLogIn.style.border = "3px solid blue";
...
<body>
    ...
    <p id="toLoad2" style="text-align: right; margin: 0px;"></p>
    <form method="POST" action="stk_logout.php" >
        <p id="toLoad3" style="text-align: right; margin: 0px;"></p>
    </form>
    <hr>
    <div class="center">
