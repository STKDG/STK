        ...
        if(!$real_pw == 0){
            if(check_password($user_pw, $real_pw)&&check_type($mysqli, $user_id)){
                ...
                if(check_title($mysqli, $cartoon_title)&&check_file($_FILES['Image']['name'][0])){
                    ...
                    if($mysqli->query($sql)){
                        $dir = mkdir("C:/Bitnami/wampstack-7.1.29-0/apache2/htdocs/STK_UNCOMPLETED_2-6(임시)/Uploaded_Images/$cartoon_title",0700,true);
                        ...
                        foreach ($_FILES['Image']['name'] as $f=>$name) { 
                            $uploaddir = "C:\Bitnami\wampstack-7.1.29-0\apache2\htdocs\STK_UNCOMPLETED_2-6(임시)\Uploaded_Images\\$cartoon_title\\";
